...
function FormEditarSolicitud() {
    ...
    const editarSolicitud = async (e) => {
        ...
        try {            
            const res = await clienteAxios.post(`/checklist/editar/${id}`, solicitud,{
                headers: {
            ...
            Swal.fire({
                title: 'Se actualizo correctamente la solicitud',
                text: res.data.msg,
                type: 'success',
                timer: 3500
            });
    ...
    const consultarAPI = async () => {
        ...
        try {
            const res = await clienteAxios.get(`checklist/solicitud/${id}`,{
                headers: {
                    Authorization: `Bearer ${auth.token}`
                }
            });
            
            guardarSolicitud(res.data);
        } catch (error) {
        ...
        <Fragment>
            <div className="card contenedor">
                ...
                <div className="card-body">
                    ...
                    <form onSubmit={editarSolicitud} onKeyDown={avanzar}>
                        ...
                        <div className='campo'>
                            ...
                            <textarea name="descripcion" id="descripcion" cols="50" rows="10" onChange={actualizarState} value={solicitud.descripcion} ></textarea>
                        </div>
                        ...
                        <div className='campo'>
                            <label htmlFor="fechaRespuesta">Fecha Respuesta<span className='campo__obligatorio'>*</span>:</label>
                            <input 
                                type="date" 
                                id='fechaRespuesta'
                                name='fechaRespuesta'
                                defaultValue={solicitud.fechaRespuesta}
                                onChange={actualizarState}
                            />
                        </div>

                        <div className='campo'>
                            <label htmlFor="version">Versión<span className='campo__obligatorio'>*</span>:</label>
                            <input 
                                type="text" 
                                id='version'
                                name='version'
                                placeholder='Versión de la solicitud'
                                onChange={actualizarState}
                                defaultValue={solicitud.version}
                            />
                        </div> 
