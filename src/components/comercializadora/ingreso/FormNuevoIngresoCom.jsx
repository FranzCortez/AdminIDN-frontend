    ...
    const consultarAPI = async (empresa) => {
        try {        
            ...
            if ( empresa === 1 ) {
                ...
                console.log(3)
                const resOvin = await clienteAxios.get('/ingresocom/ovin',{
                    headers: {
                        Authorization: `Bearer ${auth.token}`
                    }
                });
                
                console.log(4, resOvin.data)
                ingresoSpa.ovin = resOvin.data;
                guardarIngresoSpa({...ingresoSpa});
                console.log(5)
                ...
                const resOtin = await clienteAxios.get('/ih/otin',{
                    headers: {
                        Authorization: `Bearer ${auth.token}`
                    }
                });
                
                ingresoEirl.otin = resOtin.data;
                guardarIngresoEirl({...ingresoEirl});
    ...
    const agregarIngreso = async (e) => {
        ...
        try {
            if ( empresa === 1) {
                ...
                const res = await clienteAxios.post('/ingresocom', ingresoSpa,{
                    headers: {
                        Authorization: `Bearer ${auth.token}`
                    }
                });
        ...
        <section className='ingresoscom'>
            ...
            <div className='ic__body'>
                ...
                <div className={`ic__card height-auto ${empresa === 1 ? 'ic__card-com' : empresa === 2 ? 'ic__card-eirl': ''}`}>
                    ...
                    <h2 className='ic__titulo mt-50'>Crear un nuevo ingreso</h2>

                    <form className='ic__form' onSubmit={agregarIngreso} onKeyDown={avanzar}>

                        <Select value={empresa} saveValue={seleccionarEmpresa} id_name={"empresa"} label={"Seleccione Empresa"} opciones={[{ value: 1, text: 'SPA' }, { value: 2, text: 'EIRL' }]} key={1} />
...
export default FormNuevoIngresoCom