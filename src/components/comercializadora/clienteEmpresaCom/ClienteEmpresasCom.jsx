...
import ClienteEmpresaCom from './ClienteEmpresaCom.jsx';
import FormularioBuscarEmpresaCom from './FormularioBuscarEmpresaCom.jsx';
import { CRMContext } from '../../context/CRMContext.jsx';

import Spinner from '../../layout/Spinner.jsx';

function ClienteEmpresasCom() {
    ...
    const consultarAPI = async () => {
        ...
        try {
            ...
            const res = await clienteAxios.get(`clientescom/empresacom/${offset}`,{
                headers: {
                    Authorization: `Bearer ${auth.token}`
                }
            });
                            ...
                            <tbody>
                                {
                                    empresas.length > 0 ? (
                                        empresas.map((datos) => (
                                            <ClienteEmpresaCom datos={datos} key={datos.id} escucharCambio={escucharCambio}/>
                                        ))
                                    ) : 
                                    spin ? 
                                            <Spinner/>
...
export default ClienteEmpresasCom
